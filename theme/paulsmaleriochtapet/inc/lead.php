 ...
 * or photo. They call pa_lead() instead. To build a demo for a real firm, the
 ...
 *
 * With no lead-data.php present the defaults below render the fictional Pauls Måleri & Tapet.
 *
 ...
 * @return string
 */
function pa_sample_note() {
	return pa_lead( 'ar_riktig' ) ? '' : ' <span class="pa-sample">(platshållare)</span>';
}
 ...
 * @return string File name for pa_lead_img().
 */
function pa_lead_bild( $i ) {
	$bilder = (array) pa_lead( 'bilder', array() );
 ...
 * @return string
 */
function pa_ring_label() {
	$namn = pa_lead( 'kontakt' );
	return $namn ? sprintf( 'Ring %s · %s', $namn, pa_lead( 'telefon' ) ) : 'Ring ' . pa_lead( 'telefon' );
}
