...
add_action(
	...
	function () {
		register_post_type(
			...
			array(
				'labels'          => array(
					'name'          => __( 'Leads', 'paulsmaleriochtapet-leads' ),
					'singular_name' => __( 'Lead', 'paulsmaleriochtapet-leads' ),
					'edit_item'     => __( 'Förfrågan', 'paulsmaleriochtapet-leads' ),
					'search_items'  => __( 'Search leads', 'paulsmaleriochtapet-leads' ),
					'not_found'     => __( 'Inga förfrågningar än. Skicka formuläret så dyker den upp här.', 'paulsmaleriochtapet-leads' ),
					'all_items'     => __( 'Alla förfrågningar', 'paulsmaleriochtapet-leads' ),
				),
...
add_filter(
	...
	function () {
		return array(
			...
			'title'        => __( 'Name', 'paulsmaleriochtapet-leads' ),
			'npl_phone'    => __( 'Phone', 'paulsmaleriochtapet-leads' ),
			'npl_service'  => __( 'Needs', 'paulsmaleriochtapet-leads' ),
			'npl_timeline' => __( 'When', 'paulsmaleriochtapet-leads' ),
			'npl_zip'      => __( 'ZIP', 'paulsmaleriochtapet-leads' ),
			'npl_source'   => __( 'Source', 'paulsmaleriochtapet-leads' ),
			'npl_status'   => __( 'Status', 'paulsmaleriochtapet-leads' ),
			'date'         => __( 'Received', 'paulsmaleriochtapet-leads' ),
		);
	}
);
...
function npl_source_summary( $post_id ) {
	...
	return __( 'Direct', 'paulsmaleriochtapet-leads' );
}
...
add_action(
	...
	function () {
		add_meta_box( 'npl_details', __( 'What they told us', 'paulsmaleriochtapet-leads' ), 'npl_details_box', 'npl_lead', 'normal', 'high' );
		add_meta_box( 'npl_status', __( 'Status', 'paulsmaleriochtapet-leads' ), 'npl_status_box', 'npl_lead', 'side', 'high' );
	}
);
...
function npl_details_box( $post ) {
	$rows = array(
		__( 'Name', 'paulsmaleriochtapet-leads' )     => get_post_meta( $post->ID, 'name', true ),
		__( 'Phone', 'paulsmaleriochtapet-leads' )    => get_post_meta( $post->ID, 'phone', true ),
		__( 'E-post', 'paulsmaleriochtapet-leads' )    => get_post_meta( $post->ID, 'email', true ),
		__( 'ZIP', 'paulsmaleriochtapet-leads' )      => get_post_meta( $post->ID, 'zip', true ),
		__( 'Needs', 'paulsmaleriochtapet-leads' )    => npl_label( 'service', get_post_meta( $post->ID, 'service', true ) ),
		__( 'When', 'paulsmaleriochtapet-leads' )     => npl_label( 'timeline', get_post_meta( $post->ID, 'timeline', true ) ),
		__( 'Source', 'paulsmaleriochtapet-leads' )   => npl_source_summary( $post->ID ),
	);
	...
	$rows[ __( 'Samtycke lämnat', 'paulsmaleriochtapet-leads' ) ] = get_post_meta( $post->ID, 'consent_at', true ) . ' — "' . get_post_meta( $post->ID, 'consent_text', true ) . '"';
...
function npl_status_box( $post ) {
	...
	echo '<label class="screen-reader-text" for="npl-status">' . esc_html__( 'Status', 'paulsmaleriochtapet-leads' ) . '</label><select id="npl-status" name="npl_status" style="width:100%">';
