...
add_filter(
	...
	function ( $exporters ) {
		$exporters['paulsmaleriochtapet-leads'] = array(
			'exporter_friendly_name' => __( 'Quote requests', 'paulsmaleriochtapet-leads' ),
			'callback'               => function ( $email ) {
				$items = array();
				foreach ( npl_leads_by_email( $email ) as $lead ) {
					$data = array( array( 'name' => __( 'Received', 'paulsmaleriochtapet-leads' ), 'value' => $lead->post_date ) );
					...
					$items[] = array(
						'group_id'    => 'paulsmaleriochtapet-leads',
						'group_label' => __( 'Quote requests', 'paulsmaleriochtapet-leads' ),
...
add_filter(
	...
	function ( $erasers ) {
		$erasers['paulsmaleriochtapet-leads'] = array(
			'eraser_friendly_name' => __( 'Quote requests', 'paulsmaleriochtapet-leads' ),
			'callback'             => function ( $email ) {
				$removed = false;
				foreach ( npl_leads_by_email( $email ) as $lead ) {
